...
function microsoftFailureDetails(error){
  const raw=String(error?.message||"Falha não identificada na autorização Microsoft");
  const aadsts=raw.match(/\bAADSTS\d+\b/i)?.[0];
  const code=String(error?.code||aadsts||"microsoft_oauth_error").slice(0,100);
  const message=raw.replace(/\s*Trace ID:[\s\S]*$/i,"").trim().slice(0,500);
  return{code,message};
}
...
app.get("/api/microsoft/callback",async(req,res)=>{
  const authorizationCode=String(req.query.code||""),state=String(req.query.state||"");
  let userId=null;
  try{
    if(!state)throw Object.assign(new Error("A Microsoft não devolveu o identificador de segurança da conexão"),{code:"missing_state"});
    const saved=(await pool.query(`DELETE FROM oauth_states WHERE state_hash=$1 AND expires_at>NOW() RETURNING user_id`,[tokenHash(state)])).rows[0];
    userId=saved?.user_id||null;
    if(req.query.error)throw Object.assign(new Error(String(req.query.error_description||req.query.error)),{code:String(req.query.error)});
    if(!saved||!authorizationCode)throw Object.assign(new Error("Autorização expirada ou inválida. Inicie a conexão novamente."),{code:"invalid_or_expired_state"});
    const response=await fetch(`https://login.microsoftonline.com/${process.env.MICROSOFT_TENANT_ID}/oauth2/v2.0/token`,{method:"POST",headers:{"content-type":"application/x-www-form-urlencoded"},body:new URLSearchParams({client_id:process.env.MICROSOFT_CLIENT_ID||"",client_secret:process.env.MICROSOFT_CLIENT_SECRET||"",code:authorizationCode,redirect_uri:microsoftRedirectUri(),grant_type:"authorization_code",scope:"offline_access User.Read Mail.Read Mail.Send"})});
    const tokenText=await response.text();
    let token={};
    try{token=tokenText?JSON.parse(tokenText):{};}catch{throw Object.assign(new Error(`A Microsoft retornou uma resposta inválida ao trocar a autorização (HTTP ${response.status})`),{code:"invalid_token_response"});}
    if(!response.ok)throw Object.assign(new Error(token.error_description||token.error||"Falha ao obter a autorização Microsoft"),{code:token.error||`token_http_${response.status}`});
    if(!token.access_token)throw Object.assign(new Error("A Microsoft não devolveu o token de acesso esperado"),{code:"missing_access_token"});
    if(!token.refresh_token)throw Object.assign(new Error("A Microsoft não autorizou acesso contínuo. Confirme a permissão offline_access e tente novamente."),{code:"missing_refresh_token"});
    ...
    let profile={};try{profile=profileText?JSON.parse(profileText):{};}catch{}
    if(!profileResponse.ok)throw Object.assign(new Error(profile.error?.message||`Não foi possível ler o perfil Microsoft (HTTP ${profileResponse.status})`),{code:profile.error?.code||"profile_error"});
    ...
    res.redirect(`${frontendUrl}/?microsoft=connected`);
  }catch(error){
    const failure=microsoftFailureDetails(error);
    console.error(`[Microsoft OAuth] ${failure.code}: ${failure.message}`);
    if(userId)await saveConnectionStatus(userId,"failed",`${failure.code}: ${failure.message}`).catch(()=>{});
    res.redirect(`${frontendUrl}/?microsoft=error&code=${encodeURIComponent(failure.code)}&detail=${encodeURIComponent(failure.message)}`);
  }
});
